...
static inline void databus_init(PIO, int, uint, pio_sm_config *, int pin_base,
                                int pin_count, int sideset_pin_base,
                                int sideset_pin_count, bool output);
...
Programmer::Programmer(const eeprom_info_t &info) {
  ...
  // We select pio slice to use, we migth want to dynamically select one in the
  // future instead of hardcoding it.
  pio = pio0;
  ...
  addressbus_init(pio, addressbus_sm, addressbus_offset, ADDRESSBUS_BASE_PIN,
                  ADDRESSBUS_PIN_COUNT);
  ...
  // Initializes and starts databus state machine with required configs for
  // selected op_mode
  ...
  databus_init(pio, databus_sm, databus_offset, &databus_config,
               DATABUS_BASE_PIN, DATABUS_PIN_COUNT, DATABUS_SIDESET_BASE_PIN,
               DATABUS_SIDESET_PIN_COUNT, false);
...
void Programmer::set_mode(const OP_MODE mode) {
  ...
  databus_init(pio, databus_sm, databus_offset, &databus_config,
               DATABUS_BASE_PIN, DATABUS_PIN_COUNT, DATABUS_SIDESET_BASE_PIN,
               DATABUS_SIDESET_PIN_COUNT,
               (op_mode == OP_MODE::READ) ? false : true);
...
void Programmer::init_mode(const OP_MODE mode) {
  switch (mode) {
  case OP_MODE::READ:
    pio_sm_put_blocking(pio, databus_sm,
                        eeprom_info.read_delay_ns / DATABUS_PERIOD_NS);
    break;
  case OP_MODE::WRITE:
    pio_sm_put_blocking(pio, databus_sm,
                        eeprom_info.write_delay_ns / DATABUS_PERIOD_NS);
    break;
...
void Programmer::wimage(const data_t *data, int size) {
  ...
  for (int address = 0; size > address; address++) {
    ...
    if (address % eeprom_info.page_size == 0)
      sleep_ms(eeprom_info.page_delay_ms);
...
void Programmer::rimage(data_t *data, int &size) {
  ...
  for (int address = 0; size > address; address++) {
    pio_sm_put_blocking(pio, addressbus_sm, address & eeprom_info.address_mask);
    data[address] =
        (data_t)pio_sm_get_blocking(pio, databus_sm) & eeprom_info.data_mask;
  }
};
...
static inline void addressbus_init(PIO pio, int sm, uint offset, int pin_base,
                                   int pin_count) {
  // Setup pin mapping
...
static inline void databus_init(PIO pio, int sm, uint offset, pio_sm_config *c,
                                int pin_base, int pin_count,
                                int sideset_pin_base, int sideset_pin_count,
                                bool output) {
  uint pin_mask = 0x00, pin_dirs = 0x00;
  ...
  // Sets State Machine clock divider, normally runs at 100Mhz which gives a
  // 10ns clock period making every instruction in the state machine complete in
